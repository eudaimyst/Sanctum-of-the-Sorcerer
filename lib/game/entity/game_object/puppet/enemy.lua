...
local function enemyOnFrame(self)
	...
	elseif self.bloodStarted ~= true then --not already started blood splatter
		self:bloodSplatterEmitter()
		self.bloodStarted = true       --state is death
	end
end
